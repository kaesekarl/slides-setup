...
class NumberedList(VMobject):
    ...
    def __make_data(self, bulletpoints_list:list[str]):
        ...
        for i, item in enumerate(bulletpoints_list):
            ...
            number = f"|g{i+1}."
            number = Text(number, font=c.NumberedList.font, font_size=c.NumberedList.font_size, line_spacing=0.5)
            number[0:2].set_opacity(0)

            text = "|g" + item
            text = Text(text, font=c.NumberedList.font, font_size=c.NumberedList.font_size, line_spacing=0.5)
